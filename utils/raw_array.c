...
/* IMPORTANT! ==> Improve error handling for NULL pointers (rewrite the error message in 'error_handler.c') */

void init_arr(raw_array *arr, int n) {
    if (0 > n) raise(NEGATIVE_N_PACKETS, 1, n);

    arr->values = (void **)malloc(n * sizeof(void *));
    if (NULL == arr->values) raise(NULL_POINTER, 1, "arr->values", __FILE__);
...
void allocate(raw_array *arr, int n) {
    ...
    if (0 == arr->allocated) init_arr(arr, n);
        ...
        if (NULL == new_ptr) raise(NULL_POINTER, 1, "new_ptr", __FILE__);
...
void insert(raw_array *arr, void *ptr_bytes) {
    if (NULL == arr) raise(NULL_POINTER, 0, "arr", __FILE__);
...
void * get_pkt(raw_array *arr, size_t index) {
    if (NULL == arr) raise(NULL_POINTER, 0, "arr", __FILE__);
    if (NULL == arr->values) raise(NULL_POINTER, 0, "arr->values", __FILE__);
    if (index >= arr->len || index < 0) raise(INDEX_OUT_OF_BOUNDS, 0, index, arr->len);
